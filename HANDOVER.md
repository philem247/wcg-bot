...
4. **🧩 Riddle Quest Game Mode (`/riddle`)**:
   - **Engine**: `engine/riddle.js` pure state machine with 20s countdown per riddle, clean `⏳ *10 seconds remaining!*` alert (no letter/definition giveaways), and 3s intermission.
   - **Dataset**: `data/riddles.json` containing **1,050 completely distinct, non-duplicate riddles** across Classic Objects, Nature, Weather, Animals, Food, Body, African Folklore in English, and Wordplay.
